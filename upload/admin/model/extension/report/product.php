...
class ModelExtensionReportProduct extends Model {
    public function getProductsViewed(array $data = []): array {
        $sql = "SELECT pd.`name`, p.`model`, p.`viewed` FROM `" . DB_PREFIX . "product` p LEFT JOIN `" . DB_PREFIX . "product_description` pd ON (p.`extension_id` = pd.`extension_id`) WHERE pd.`language_id` = '" . (int)$this->config->get('config_language_id') . "' AND p.`viewed` > '0' ORDER BY p.`viewed` DESC";
    ...
    public function getPurchased(array $data = []): array {
        ...
        $sql .= " GROUP BY op.`extension_id` ORDER BY total DESC";
    ...
    public function getTotalPurchased(array $data): int {
        $sql = "SELECT COUNT(DISTINCT op.`extension_id`) AS `total` FROM `" . DB_PREFIX . "order_product` op LEFT JOIN `" . DB_PREFIX . "order` o ON (op.`order_id` = o.`order_id`)";
