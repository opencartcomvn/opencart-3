    ...
    public function popup_product(): void {
        ...
        if ($this->request->post['operand']['type'] == 'single') {
            ...
            if ($product_info !== null) {
                ...
                $required_fields = $this->model_extension_advertise_google->getRequiredFieldsByProductIds([$product_info['extension_id']], $this->store_id);
                ...
                $options = $this->model_extension_advertise_google->getProductOptionsByProductIds([$product_info['extension_id']]);
    ...
    public function copyProduct(string &$route, array &$args, mixed &$output): void {
        ...
        if (!empty($final_extension_id)) {
            foreach ($this->model_catalog_product->getProductStores($final_extension_id) as $store_id) {
                $this->model_extension_advertise_google->insertNewProducts([$final_extension_id], $store_id);
            }
        }
    }
            ...
            foreach ($required_fields as $key => $requirements) {
                if (empty($requirements['selected_field']) && (!isset($this->request->post['form'][$key]) || $this->request->post['form'][$key] == '')) {
                    ...
                    foreach ($requirements['selected_field'] as $dependency => $values) {
                        if (in_array($this->request->post['form'][$dependency], (array)$values) && (!isset($this->request->post['form'][$key]) || $this->request->post['form'][$key] == '')) {
                            $this->error[$key] = $this->language->get('error_field_no_value');
                        }
                    }
                }
