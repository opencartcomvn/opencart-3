...
class ControllerExtensionPaymentPPExpress extends Controller {
    ...
    public function express(): void {
        ...
        if ($this->customer->isLogged()) {
            ...
            if ($this->config->get('config_checkout_guest') && !$this->config->get('config_customer_price') && !$this->cart->hasDownload() && !$this->cart->hasSubscription()) {
                /**
        ...
        if (isset($this->session->data['pp_login']['seamless']['access_token']) && (isset($this->session->data['pp_login']['seamless']['customer_id']) && $this->session->data['pp_login']['seamless']['customer_id'] == $this->customer->getId()) && $this->config->get('module_pp_login_seamless')) {
            $post_data['IDENTITYACCESSTOKEN'] = $this->session->data['pp_login']['seamless']['access_token'];
        }
        ...
        if ($redirect == '') {
            ...
            if ($result['ACK'] == 'Success') {
                ...
                if ($subscription_products) {
                    ...
                    foreach ($subscription_products as $item) {
                        $data = [
                            ...
                            'BILLINGFREQUENCY'   => $item['subscription']['cycle'],
                            'TOTALBILLINGCYCLES' => $item['subscription']['duration'],
                            'AMT'                => $this->currency->format($this->tax->calculate($item['subscription']['price'], $item['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency'], false, false) * $item['quantity'],
                        ...
                        if ($item['subscription']['trial_status']) {
                            $data_trial = [
                                'TRIALBILLINGPERIOD'      => $billing_period[$item['subscription']['trial_frequency']],
                                'TRIALBILLINGFREQUENCY'   => $item['subscription']['trial_cycle'],
                                'TRIALTOTALBILLINGCYCLES' => $item['subscription']['trial_duration'],
                ...
                if (isset($result['REDIRECTREQUIRED']) && $result['REDIRECTREQUIRED'] == true) {
                    //- Handles German redirection here
    ...
    public function checkout(): void {
        ...
        $post_data = array_merge($post_data, $data_shipping);
    ...
    public function ipn(): void {
        ...
        if ((string)$response == 'VERIFIED') {
            ...
            if (isset($this->request->post['txn_type'])) {
                ...
                } elseif (isset($this->request->post['parent_txn_id'])) {
            ...
            $this->model_extension_payment_pp_express->log(['IPN was invalid'], 'IPN fail');
        } else {
