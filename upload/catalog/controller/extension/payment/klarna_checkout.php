...
class ControllerExtensionPaymentKlarnaCheckout extends Controller {
    ...
    public function main(): void {
        ...
        if (!$redirect) {
            ...
            ] = $this->klarnaOrderData($klarna_account);
            ...
            if ($klarna_checkout) {
                $this->session->data['klarna_checkout_order_id'] = (int)$klarna_checkout['order_id'];
            ...
            if ($country_info) {
                ...
                if ($zone || $zones) {
                    $this->session->data['shipping_address'] = [
                        'address_id'     => null,
                        'firstname'      => oc_substr($this->request->post['given_name'], 0, 32),
                        'lastname'       => oc_substr($this->request->post['family_name'], 0, 32),
                        'company'        => null,
                        'address_1'      => oc_substr($this->request->post['street_address'], 0, 128),
                        'address_2'      => oc_substr($this->request->post['street_address'], 129, 256),
                        'postcode'       => oc_substr($this->request->post['postal_code'], 0, 10),
                        'city'           => oc_substr($this->request->post['city'], 0, 128),
        ...
        if ($process) {
            ...
            if ($order_info && $country_info) {
                ...
                if ($zone || $zones) {
                    ...
                    $this->session->data['klarna_checkout_data'] = [
    ...
    public function validation(): void {
        ...
        if ($validate) {
            if (!$this->customer->isLogged()) {
                $this->model_extension_payment_klarna_checkout->updateOcOrderEmail($order_id, oc_substr($request->shipping_address->email, 0, 96));
            }
            ...
            $order_data = [
                'firstname'               => oc_substr($request->billing_address->given_name, 0, 32),
                'lastname'                => oc_substr($request->billing_address->family_name, 0, 32),
                'telephone'               => oc_substr($request->billing_address->phone, 0, 32),
                'payment_firstname'       => oc_substr($request->billing_address->given_name, 0, 32),
                'payment_lastname'        => oc_substr($request->billing_address->family_name, 0, 32),
                'payment_address_1'       => oc_substr($request->billing_address->street_address, 0, 128),
                'payment_address_2'       => isset($request->billing_address->street_address2) ? oc_substr($request->billing_address->street_address2, 0, 128) : '',
                'payment_city'            => oc_substr($request->billing_address->city, 0, 128),
                'payment_postcode'        => oc_substr($request->billing_address->postal_code, 0, 10),
                ...
                'shipping_firstname'      => oc_substr($request->shipping_address->given_name, 0, 32),
                'shipping_lastname'       => oc_substr($request->shipping_address->family_name, 0, 32),
                'shipping_address_1'      => oc_substr($request->shipping_address->street_address, 0, 128),
                'shipping_address_2'      => isset($request->shipping_address->street_address2) ? oc_substr($request->shipping_address->street_address2, 0, 128) : '',
                'shipping_city'           => oc_substr($request->shipping_address->city, 0, 128),
                'shipping_postcode'       => oc_substr($request->shipping_address->postal_code, 0, 10),
                    ...
                    if ($klarna_account && $connector) {
                        ...
                        if ($order) {
                            if ($order->acknowledge()) {
                                ...
                                $order_data = [
                                    'firstname'               => oc_substr($order['billing_address']['given_name'], 0, 32),
                                    'lastname'                => oc_substr($order['billing_address']['family_name'], 0, 32),
                                    'telephone'               => oc_substr($order['billing_address']['phone'], 0, 32),
                                    'payment_firstname'       => oc_substr($order['billing_address']['given_name'], 0, 32),
                                    'payment_lastname'        => oc_substr($order['billing_address']['family_name'], 0, 32),
                                    'payment_address_1'       => oc_substr($order['billing_address']['street_address'], 0, 128),
                                    'payment_address_2'       => isset($order['billing_address']['street_address2']) ? oc_substr($order['billing_address']['street_address2'], 0, 128) : '',
                                    'payment_city'            => oc_substr($order['billing_address']['city'], 0, 128),
                                    'payment_postcode'        => oc_substr($order['billing_address']['postal_code'], 0, 10),
                                    'payment_zone'            => ($payment_zone_info ? $payment_zone_info['name'] : ''),
                                    'payment_zone_id'         => ($payment_zone_info ? $payment_zone_info['zone_id'] : ''),
                                    'payment_country'         => ($payment_country_info ? $payment_country_info['name'] : ''),
                                    'payment_country_id'      => ($payment_country_info ? $payment_country_info['country_id'] : ''),
                                    'payment_address_format'  => ($payment_country_info ? $payment_country_info['address_format'] : ''),
                                    'shipping_firstname'      => oc_substr($order['shipping_address']['given_name'], 0, 32),
                                    'shipping_lastname'       => oc_substr($order['shipping_address']['family_name'], 0, 32),
                                    'shipping_address_1'      => oc_substr($order['shipping_address']['street_address'], 0, 128),
                                    'shipping_address_2'      => isset($order['shipping_address']['street_address2']) ? oc_substr($order['shipping_address']['street_address2'], 0, 128) : '',
                                    'shipping_city'           => oc_substr($order['shipping_address']['city'], 0, 128),
                                    'shipping_postcode'       => oc_substr($order['shipping_address']['postal_code'], 0, 10),
                                    'shipping_zone'           => ($shipping_zone_info ? $shipping_zone_info['name'] : ''),
                                    'shipping_zone_id'        => ($shipping_zone_info ? $shipping_zone_info['zone_id'] : ''),
                                    'shipping_country'        => ($shipping_country_info ? $shipping_country_info['name'] : ''),
                                    'shipping_country_id'     => ($shipping_country_info ? $shipping_country_info['country_id'] : ''),
    ...
    private function klarnaOrderData($klarna_account) {
        ...
        $encrypted_order_data = $this->encryption->encrypt($this->config->get('config_encryption'), json_encode([
        ...
        $klarna_order_data['merchant_reference1'] = (int)$this->session->data['order_id'];
        $klarna_order_data['options'] = [];
        ...
        if ($this->config->get('payment_klarna_checkout_additional_text_box') && $this->customer->isLogged() && !$this->customer->getNewsletter()) {
            $klarna_order_data['options']['additional_checkbox'] = [
